			...
			if (gl_hMapWeaponProperties.GetArray(szWeaponName, iWeaponProperty, C_WEAPON_PROPERTY_MAXIMUM))
			{
				...
				if (bGround || !(GetEntityFlags(iPlayer) & FL_ONGROUND))
				{
					...
					if (distance <= maxDistance)
					{
						...
						PrintToServer("X %0.2f | Y %0.2f | Z %0.2f", gl_vPlayerWeaponJumpVelocity[iPlayer][0], gl_vPlayerWeaponJumpVelocity[iPlayer][1], gl_vPlayerWeaponJumpVelocity[iPlayer][2]);
						...
						EmitAmbientSound("murlisgib/blast_jump.wav", impact_pos, SOUND_FROM_WORLD, 85, SND_NOFLAGS, 0.7, SNDPITCH_NORMAL);
